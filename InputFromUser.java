...
public class InputFromUser {
    ...
    public User inputFromUser() throws UserException {
        while (true) {
            System.out.println("Введите через пробел: \n" +
                    ...
                    "пол --> m или f, где m = мужской, f = женский\n>>> ");
            try {
                ...
                GenderType gender = parseGender(messageFromUser[5]);
    ...
    private GenderType parseGender(String genderString) throws ParseException {
        if (genderString.equalsIgnoreCase("m")) {
            return GenderType.MALE;
        } else if (genderString.equalsIgnoreCase("f")) {
            return GenderType.FEMALE;
        } else {
            throw new ParseException("Введите пол корректно m или f", 0);
        }
    }
}
